            ...
            <div className='flex flex-wrap max-w-[1512px] py-[32px] w-full'>
                <div className='max-w-[1065px] w-full'>
                    ...
                    <div className='flex flex-row items-center justify-between mt-[24px]'>
                        <p className='text-white text-[25px] max-w-[942px]'>{post.name}</p>
                        <div className='flex flex-row items-center'>
                            {
                                post.sub_mn && <p className='bg-[#400740] py-[5px] px-[7px] rounded-[10px] text-white text-[15px]'>MN</p>
                            }
                            {
                                post.sub_eng && <p className='bg-[#400740] py-[5px] px-[7px] rounded-[10px] ml-[5px] text-white text-[15px]'>EN</p>
                            }
                        </div>
                    </div>
                    <p className='text-[#A7A7A7] text-[20px] mt-[9px]'>{post.views}&nbsp;&nbsp;•&nbsp;&nbsp;{new Date(post.created_at).getFullYear()}-{new Date(post.created_at).getMonth() + 1}-{new Date(post.created_at).getDate()}</p>
                    ...
                    <div className='bg-[#2C262C] w-full rounded-[10px] mt-[19px] flex flex-row pt-[15px] px-[11px] pb-[33px]'>
                        <img style={{objectFit: 'cover'}} src={post.poster} className='w-[199px] h-[292px] rounded-[10px]'/>
                        <div className='max-w-[794px] w-full ml-[44px]'>
                            <div className='flex flex-row items-center'>
                                <p className='text-[25px] text-white'>{post.name}</p>
                                <p className='text-[#A7A7A7] text-[20px] ml-[9px]'>{post.name_japanese}</p>
                            </div>
                            <p className='text-white text-[20px] mt-[16px]'>{post.synopsis}</p>
                            <div className='flex flex-row justify-between mt-[16px]'>
                                <div className='flex flex-col'>
                                    <span className='inline-flex text-[#A7A7A7] text-[20px]'>Студи:&nbsp;<p className='text-[#CE8CD2]'>{post.studio}</p></span>
                                    <span className='inline-flex text-[#A7A7A7] text-[20px] mt-[10px]'>Гарч эхлэсэн:&nbsp;<p>{post.aired_at}</p></span>
                                    <span className='inline-flex text-[#A7A7A7] text-[20px] mt-[10px]'>Төлөв:&nbsp;<p className='text-[#CE8CD2]'>{post.status === 0 ? 'Дууссан' : 'Гарч байгаа'}</p></span>
                                </div>
                                <div className='flex flex-col'>
                                    <span className='inline-flex text-[#A7A7A7] text-[20px]'>Төрөл:&nbsp;<p className='text-[#CE8CD2]'>{post.category_id === 1 ? 'OVA' : post.category_id === 2 ? 'TV-Series' : post.category_id === 3 ? 'Movie' : post.category_id === 4 ? 'ONA' : "NULL"}</p></span>
                                    <span className='inline-flex text-[#A7A7A7] text-[20px] mt-[10px]'>Хугацаа:&nbsp;<p>{post.duration} минут</p></span>
                                    <span className='inline-flex text-[#A7A7A7] text-[20px] mt-[10px]'>Анги:&nbsp;<p>{post.current_episode}/{post.episode_list}</p></span>
                                </div>
                                <div className='bg-[#403840] rounded-[10px] px-[17px] pt-[18px] pb-[11px] flex flex-row justify-evenly'>
                                    <div>
                                        <p className='text-[#CE8CD2] text-[20px] text-center'>{post.translator}</p>
                                        <p className='text-[#A7A7A7] text-[20px] mt-[6px] text-center'>Орчуулсан</p>
                                    </div>
                                    {
                                        post.review_by &&
                                        <div>
                                            <p className='text-[#CE8CD2] text-[20px] text-center'>{post.review_by}</p>
